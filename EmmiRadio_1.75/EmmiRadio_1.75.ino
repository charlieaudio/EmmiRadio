/* -------------------------------------------------------------------------
   EmmiRadio v1.75 – stable base + long-press volume
   -------------------------------------------------------------------------
   Based on: 1.74 stable rebuild
   Main additions:
   - colored WebUI buttons restored
   - short PLAY  = play / stop
   - long  PLAY  = volume up
   - short NEXT  = next station
   - long  NEXT  = volume down
   - volume overlay stays for 3 seconds, then returns to normal display
...
String pageIndex(){
  ...
  h+="<div class='hero'><div class='title'>EmmiRadio v"+String(FW_VERSION)+"</div><div class='sub'>Stable base with sane long-press volume.</div></div><div class='grid'>";
  ...
  h+="<div class='card'>"
     ...
     "<div class='row'><label>Startup / current volume: <b id='volLabel'>"+String(currentVolume)+"</b></label>"
...
void handlePrev(){ redirectHome(); } // PREV not used
...
void setup(){
  ...
  displayInit();
  displayWake();
  safeLcdPrint("EmmiRadio v1.75","Boot...");
