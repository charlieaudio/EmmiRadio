/* -------------------------------------------------------------------------
   EmmiRadio v1.7 – live volume UI + long-press volume buttons + last station
   -------------------------------------------------------------------------
   Main changes from v1.62:
   - real-time volume update from web UI slider
   - long press PLAY = volume up, NEXT = volume down
   - volume shown on display as "Vol: xx"
   - after 3 seconds of inactivity, line 2 returns to normal duty
   - current volume saved and restored on boot
   - last selected station saved and restored on boot
...
const unsigned long LONG_REPEAT_MS = 200; // 5 units / second
inline bool rawPressed(int pin){ return digitalRead(pin)==LOW; }
DebBtn BTN_NEXT{PIN_BTN_NEXT, false, false, 0, 0, 0, false};
DebBtn BTN_PREV{PIN_BTN_PREV, false, false, 0, 0, 0, false};
DebBtn BTN_PLAY{PIN_BTN_PLAY, false, false, 0, 0, 0, false};
...
void initBtn(DebBtn& b){
  ...
  b.pressedSinceMs = r ? millis() : 0;
  b.lastRepeatMs = 0;
  b.longPressActive = false;
}

bool btnPressedEdge(DebBtn& b){
...
bool btnReleasedShort(DebBtn& b){
  unsigned long now = millis();
  bool raw = rawPressed(b.pin);
  if (raw != b.stable && (now - b.lastChangeMs) >= DEBOUNCE_MS){
    b.lastStable = b.stable;
...
String pageIndex(){
  ...
  h+="</div>"
     "<script>"
     "let volTimer=null;"
     "function liveVolume(v){document.getElementById('volLabel').innerText=v;document.getElementById('volValue').innerText=v;clearTimeout(volTimer);volTimer=setTimeout(()=>{fetch('/api/vol_set?v='+v).catch(()=>{});},120);}"
     "</script>";
  h+=htmlFooter();
  return h;
}
...
void setup(){
  ...
  displayInit();
  displayWake();
  safeLcdPrint("EmmiRadio v1.7","Boot...");
...
void loop(){
  ...
  if (btnPressedEdge(BTN_PLAY) || btnPressedEdge(BTN_NEXT) || btnPressedEdge(BTN_PREV)){
    lastActivityMs = millis();
  ...
  if (btnReleasedShort(BTN_PLAY)) {
    lastActivityMs = millis();
    if(!isPlaying) doPlay(); else doStop();
  }
  if (btnReleasedShort(BTN_NEXT)) {
    lastActivityMs = millis();
    changeStation(+1);
  }
  if (btnReleasedShort(BTN_PREV)) {
    lastActivityMs = millis();
    changeStation(-1);
  }
