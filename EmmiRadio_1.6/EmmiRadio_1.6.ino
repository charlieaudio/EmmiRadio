/* -------------------------------------------------------------------------
   EmmiRadio v1.6 – FFat + selectable LCD / OLED display backend
   -------------------------------------------------------------------------
   Main changes from v1.5:
   - LittleFS replaced with FFat
   - more informative Serial logging
   - prints IP address and mode on startup
...
String pageIndex(){
  ...
  h+="<div class='row'><b>FS:</b> FFat</div>";
...
void handleVolUp(){ currentVolume++; applyVolume(); Serial.println(String("[AUDIO] Volume -> ") + currentVolume); redirectHome(); }
void handleVolDown(){ currentVolume--; applyVolume(); Serial.println(String("[AUDIO] Volume -> ") + currentVolume); redirectHome(); }
...
void setup(){
  ...
  displayInit();
  displayWake();
  safeLcdPrint("EmmiRadio v1.6","Boot...");
  ...
  applyVolume();
  Serial.println(String("[AUDIO] I2S ready, volume = ") + currentVolume);
...
void loop(){
  ...
  if(nowConn!=wasConn){
    wasConn=nowConn;
    Serial.println(String("[WiFi] State changed. Connected = ") + (nowConn ? "yes" : "no"));
    showIpOnLcd();
    logRuntimeSummary();
  }
}
