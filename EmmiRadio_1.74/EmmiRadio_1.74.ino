/* -------------------------------------------------------------------------
   EmmiRadio v1.74 – clean rebuild from 1.62
   -------------------------------------------------------------------------
   Purpose of this release:
   - get back to a single-file build that actually compiles cleanly
   - avoid the broken 1.7x partial-copy / linker hell

   Main changes from v1.62:
   - saves last selected station to /settings.json
   - restores last selected station on boot
   - no auto play on boot
   - boot IP is shown first, then normal station/status screen
   - live web volume endpoint keeps working
   - volume overlay on display for 3 seconds after volume change

   This version intentionally stays conservative and compile-safe.
   ------------------------------------------------------------------------- */
...
bool btnPressedEvent(DebBtn& b){
  ...
  if (raw != b.stable && (now - b.lastChangeMs) >= DEBOUNCE_MS){
    ...
    b.stable = raw;
    b.lastChangeMs = now;
    if (b.lastStable == false && b.stable == true) return false;
...
String pageOtaUpdate(const String& note){
  ...
  h+="<form method='POST' action='/update' enctype='multipart/form-data'><div class='row'><input type='file' name='firmware'></div><div class='toolbar'><button class='btn' type='submit'>Upload & Flash</button><a class='btnlink' href='/'>Back</a></div></form></div>";
  h+=htmlFooter();
  return h;
}
...
String pageIndex(){
  ...
  h+="<div class='hero'><div class='title'>EmmiRadio v"+String(FW_VERSION)+"</div><div class='sub'>Clean rebuild from 1.62 to stop compiler nonsense.</div></div><div class='grid'>";
  h+="<div class='card'><div class='stat'><div class='label'>Playback</div><div class='value'>"+st+"</div></div><div class='stat'><div class='label'>Station</div><div class='value'>"+cur+"</div></div><div class='stat'><div class='label'>Metadata</div><div class='value'>"+meta+"</div></div><div class='stat'><div class='label'>IP</div><div class='value mono'>"+ip+"</div></div><div class='stat'><div class='label'>Network</div><div class='value'>"+netMode+"</div></div><div class='stat'><div class='label'>Volume</div><div class='value' id='volValue'>"+String(currentVolume)+"</div></div><div class='controls'><a class='btnlink' href='/api/prev'>Prev</a><a class='btnlink' href='/api/play'>Play</a><a class='btnlink' href='/api/stop'>Stop</a><a class='btnlink' href='/api/next'>Next</a></div></div>";
  h+="<div class='card'><form method='GET' action='/api/st_select'><div class='row'><label>Select station</label><select name='i'>"+buildStationOptions()+"</select></div><button class='btn' type='submit'>Switch Station</button></form><form method='POST' action='/settings'><div class='row'><label>Startup / current volume: <b id='volLabel'>"+String(currentVolume)+"</b></label><input type='range' min='0' max='21' step='1' name='vol' value='"+String(currentVolume)+"' oninput='liveVolume(this.value)'></div><div class='row'><label><input type='checkbox' name='ps' value='1' ";
  if(powerSaveEnabled) h += "checked";
  h += "> Save display power save (30s)</label></div><div class='toolbar'><button class='btn' type='submit'>Save Settings</button><a class='btnlink' href='/stations'>Stations</a><a class='btnlink' href='/wifi'>Wi-Fi</a><a class='btnlink' href='/update'>OTA</a></div></form></div></div><script>let vt=null;function liveVolume(v){document.getElementById('volLabel').innerText=v;document.getElementById('volValue').innerText=v;clearTimeout(vt);vt=setTimeout(()=>{fetch('/api/vol_set?v='+v).catch(()=>{});},120);}</script>";
  h+=htmlFooter();
  return h;
}
...
String pageStations(const String& note){
  ...
  for(int i=0;i<stationCount;i++){
    h+="<tr><td>"+String(i)+"</td><td>"+stations[i].name+"</td><td><div class='toolbar'><a class='btnlink' href='/stations?edit="+String(i)+"'>Edit</a><a class='btnlink' href='/api/st_del?i="+String(i)+"'>Delete</a><a class='btnlink' href='/api/st_select?i="+String(i)+"'>Select</a></div></td></tr>";
  }
  h+="</table>";
  ...
  h+="<div class='row'><input name='name' value='"+fName+"' placeholder='Name'></div><div class='row'><input name='url' value='"+fUrl+"' placeholder='URL'></div><div class='toolbar'><button class='btn' type='submit'>"+String(edit>=0?"Save":"Add")+"</button><a class='btnlink' href='/'>Back</a></div></form></div>";
  h+=htmlFooter();
  return h;
}
...
String pageWifiForm(const String& note){
  ...
  for(int i=0;i<wifiCount;i++) h+="<tr><td>"+String(i)+"</td><td>"+wifiList[i].ssid+"</td><td><a class='btnlink' href='/api/wifi_del?i="+String(i)+"'>Delete</a></td></tr>";
  if(wifiCount==0) h+="<tr><td colspan='3'>(no saved networks)</td></tr>";
  h+="</table><form method='POST' action='/wifi'><div class='row'><input name='ssid' placeholder='SSID'></div><div class='row'><input name='pass' placeholder='Password'></div><div class='toolbar'><button class='btn' type='submit'>Save & Try Connect</button><a class='btnlink' href='/'>Back</a></div></form></div>";
  h+=htmlFooter();
  return h;
}
...
String fetchIcyTitleOnce(const String& url, uint32_t overallTimeoutMs){
  ...
  while(metaLen>0 && (millis()-t0)<overallTimeoutMs){
    while(client.available() && metaLen>0){ char c=(char)client.read(); if(c!='\0') meta += c; metaLen--; }
  ...
  String title = meta.substring(a,b); title.trim(); return title;
}
...
void handleVolUp(){ currentVolume++; applyVolume(); saveSettings(); showVolumeOverlay(); redirectHome(); }
void handleVolDown(){ currentVolume--; applyVolume(); saveSettings(); showVolumeOverlay(); redirectHome(); }
...
void handleWifiDel(){ if(!server.hasArg("i")){ redirectHome(); return; } int i=server.arg("i").toInt(); if(i<0||i>=wifiCount){ redirectHome(); return; } for(int k=i;k<wifiCount-1;k++) wifiList[k]=wifiList[k+1]; wifiCount--; saveWiFiList(); redirectHome(); }

void handleStationsGet(){ server.send(200,"text/html",pageStations("")); }
void handleStationsPost(){
  String name=server.arg("name"), url=server.arg("url"); name.trim(); url.trim();
...
void handleStDel(){ if(!server.hasArg("i")){ redirectHome(); return; } int i=server.arg("i").toInt(); if(i<0||i>=stationCount){ redirectHome(); return; } for(int k=i;k<stationCount-1;k++) stations[k]=stations[k+1]; stationCount--; if(currentStation>=stationCount) currentStation=0; saveStations(); saveSettings(); redirectHome(); }
void handleStSelect(){ if(!server.hasArg("i")){ redirectHome(); return; } int i=server.arg("i").toInt(); if(i<0||i>=stationCount){ redirectHome(); return; } selectStation(i); if(isPlaying){ audio.stopSong(); delay(50); lastTitle=""; gotMeta=false; playStartMs=millis(); marqueeActive=false; audio.connecttohost(stations[currentStation].url.c_str()); } showNowPlaying(); redirectHome(); }
...
bool connectWiFiSmart(unsigned long timeoutPerNetMs, int maxAttempts) {
  ...
  bool knownVisible[MAX_WIFI]; for (int i=0;i<MAX_WIFI;i++) knownVisible[i]=false;
  int knownCount=0;
  for (int s = 0; s < n; s++){
    String ssidScan = WiFi.SSID(s); ssidScan.trim();
    for (int i = 0; i < wifiCount; i++) if (!knownVisible[i] && ssidScan == wifiList[i].ssid){ knownVisible[i]=true; knownCount++; break; }
  }
  WiFi.scanDelete();
  ...
  for (int attempt = 0; attempt < maxAttempts; attempt++) {
    for (int i = 0; i < wifiCount; i++) {
      ...
      WiFi.disconnect(); delay(200); WiFi.begin(wifiList[i].ssid.c_str(), wifiList[i].pass.c_str());
      unsigned long t0 = millis(); wl_status_t st = WiFi.status();
      while (st != WL_CONNECTED && (millis() - t0) < timeoutPerNetMs) { delay(250); st = WiFi.status(); }
...
void setup(){
  ...
  displayInit();
  displayWake();
  safeLcdPrint("EmmiRadio v1.74","Boot...");
  if(!FFat.begin(true)){ safeLcdPrint("FFat error", "Mount failed"); delay(1500); }
  loadSettings();
  initBtn(BTN_PLAY); initBtn(BTN_NEXT); initBtn(BTN_PREV);
  audio.setPinout(PIN_I2S_BCK, PIN_I2S_LRCK, PIN_I2S_DATA);
  applyVolume();
  if(!loadStations()){ loadDefaultStations(); saveStations(); }
  if(currentStation >= stationCount) currentStation = 0;
...
void loop(){
  ...
  if(volumeOverlayUntilMs && millis() > volumeOverlayUntilMs){ volumeOverlayUntilMs = 0; showNowPlaying(); }
  tickMarquee();
  if (powerSaveEnabled && !lcdBacklightOff && (millis()-lastActivityMs > LCD_SLEEP_MS)){ displaySleep(); lcdBacklightOff = true; }
  if (btnPressedEvent(BTN_PLAY)){ lastActivityMs = millis(); if(lcdBacklightOff){ displayWake(); lcdBacklightOff=false; } if(!isPlaying) doPlay(); else doStop(); }
  if (btnPressedEvent(BTN_NEXT)){ lastActivityMs = millis(); if(lcdBacklightOff){ displayWake(); lcdBacklightOff=false; } changeStation(+1); }
  if (btnPressedEvent(BTN_PREV)){ lastActivityMs = millis(); if(lcdBacklightOff){ displayWake(); lcdBacklightOff=false; } changeStation(-1); }
  if(isPlaying && !gotMeta && volumeOverlayUntilMs == 0){ static unsigned long lastRefresh=0; if(millis()-lastRefresh>1000){ showNowPlaying(); lastRefresh=millis(); } }
  tryIcyFallback();
}
...
void audio_showstreamtitle(const char* t){
  ...
  if(nt!=lastTitle){ lastTitle=nt; gotMeta=true; Serial.print("[META/CALLBACK] "); Serial.println(lastTitle); showNowPlaying(); }
}
