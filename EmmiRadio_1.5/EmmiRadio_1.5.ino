/* -------------------------------------------------------------------------
   EmmiRadio v1.5 – LittleFS + selectable LCD / OLED display backend
   -------------------------------------------------------------------------
   Main changes from v1.4:
   - SPIFFS replaced with LittleFS
   - display libraries included only for selected display type
   - filesystem notes updated
...
bool loadSettings(){
  ...
  File f = LittleFS.open(SETTINGS_PATH, "r"); if(!f) return false;
...
bool saveSettings(){
  File f = LittleFS.open(SETTINGS_PATH, "w"); if(!f) return false;
...
bool loadStations(){
  stationCount=0;
  if(!LittleFS.exists(RADIOS_PATH)) return false;
  File f=LittleFS.open(RADIOS_PATH,"r"); if(!f) return false;
...
bool saveStations(){
  File f=LittleFS.open(RADIOS_PATH,"w"); if(!f) return false;
...
String pageIndex(){
  ...
  h+="<div class='row'><b>FS:</b> LittleFS</div>";
...
void setup(){
  ...
  displayInit();
  displayWake();
  safeLcdPrint("EmmiRadio v1.5","Boot...");

  if(!LittleFS.begin(true)){
    Serial.println("[FS] LittleFS mount failed");
    safeLcdPrint("LittleFS error", "Mount failed");
    delay(1500);
  } else {
